...
# These columns are used to *construct* the risk label below (price
# extremity + listing-quality thresholds). They must never appear in the
# feature list, otherwise the classifier just learns to re-derive the rule
# that built its own label instead of predicting risk from independent
# signals. See build_risk_labels() and test_train_risk_model.py.
...
def clean_input_data(df: pd.DataFrame) -> pd.DataFrame:
    df = df.dropna(subset=["listed_price", "size_sqm", "property_type"]).copy()
...
def build_risk_labels(df: pd.DataFrame) -> pd.DataFrame:
    """Assign a risk_label using only price extremity and listing-quality
    columns. These columns are intentionally excluded from the model's
    feature list (see NUMERIC_FEATURES) to avoid data leakage."""
    ...
    # If suspicious class is still empty, force the most extreme price rows
    # into suspicious. This keeps the model useful for a 3-class product demo.
...
def main() -> None:
    ...
    risk_model = build_pipeline(numeric_features, categorical_features)
